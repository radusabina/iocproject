...
function CongratsPage() {
  const mister = sessionStorage.getItem("mister");
  const operation = sessionStorage.getItem("operation");
  const navigate = useNavigate();

  const handlePlayAgainClick = () => {
    sessionStorage.setItem("mister", mister);
    navigate(operation);
  };

  const handleChangeMisteryClick = () => {
    sessionStorage.setItem("operation", operation);
    navigate("/ChooseMistery");
  };

  const handleChangeOperationClick = () => {
    navigate("/");
  };
  ...
  return (
    <div className="CongratsPage">
      <header className="app-header">
        <img
          src="https://i.postimg.cc/MHPRcxc5/temp-Image-SHw-Se-P.jpg"
          className="congrats-logo"
          alt="logo"
        />
        <div className="congrats-text">
          Felicitări! Ai rezolvat misterul
          <br />
        </div>

        <img
          src="https://i.postimg.cc/FRFRbNpt/temp-Imager-Ad-RHb.jpg"
          className="scooby-picture-congrats"
          alt="scooby-picture"
        />

        <div className="button-container">
          <button onClick={() => handlePlayAgainClick()}>Joacă din nou</button>
          <button onClick={() => handleChangeMisteryClick()}>
            Alege alt mister
          </button>
