...
function AdunarePage() {
  ...
  const calculeazaRezultat = () => {
    ...
    setRezultat(suma.toString());
    // Verificăm dacă rezultatul este corect și setăm indicatorul
    setRezultatCorect(suma === parseInt(rezultat, 10));
  };
  ...
  return (
    <div className="AdunarePage">
      ...
      <h2>Adunare</h2>
      <p>
        Calculează suma următoarelor două numere și scrie rezultatul. Dacă ai
        nevoie de ajutor, poți apăsa butonul pentru a genera noi numere.
      </p>
      <div>
        <strong>{numar1}</strong> + <strong>{numar2}</strong> =
        <input
          type="number"
          value={rezultat}
